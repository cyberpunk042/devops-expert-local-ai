...
cmd_copy() {
    ...
    if [[ -n "$P2P_TOKEN" ]]; then
        ...
        ssh "$REMOTE_HOST" "cd ~/devops-expert-local-ai && \
            grep -q '^LOCALAI_P2P_TOKEN=' .env 2>/dev/null && \
            sed -i 's|^LOCALAI_P2P_TOKEN=.*|LOCALAI_P2P_TOKEN=$P2P_TOKEN|' .env || \
            { echo '' >> .env; echo 'LOCALAI_P2P_TOKEN=$P2P_TOKEN' >> .env; echo 'LOCALAI_P2P_NETWORK_ID=aicp-fleet' >> .env; }"
