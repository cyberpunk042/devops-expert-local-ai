...
PYEOF
    ...
    echo ""
    echo "  2. Start LocalAI with P2P on this machine:"
    echo "     make local-down && make local-up-p2p"
    echo ""
    echo "  3. Start the agent daemon:"
    echo "     make agent-up"
    echo ""
    echo "  4. Add the other machine:"
    echo "     make fleet-join HOST=<ip>"
    echo ""
    echo "  5. Copy config to the other machine:"
    echo "     make fleet-copy HOST=<ip>"
    echo "     Then on the remote machine (same command — all nodes are equal):"
    echo "       make local-down && make local-up-p2p"
    echo "       make agent-up"
    echo ""
    echo "  6. Verify:"
...
cmd_firewall() {
    ...
    if [[ "$is_wsl" -eq 1 ]]; then
        ...
        echo ""
        echo "  In standalone mode, Docker auto-exposes 8090 to LAN."
        echo "  In P2P mode (network_mode: host), ALL ports need forwarding."
        echo ""
        echo "  Agent (9100) and P2P (9876) always need forwarding in WSL:"
        echo ""
        echo "    make wsl-forward          # Forward 9100 + 9876"
        echo "    make wsl-forward-check    # Verify current forwards"
        echo ""
        echo "  If using P2P mode, also forward 8090:"
        echo "    scripts/wsl-port-forward.sh 8090 9100 9876"
        echo ""
        echo "  Port forward + firewall rule are BOTH needed."
        echo ""
    fi
